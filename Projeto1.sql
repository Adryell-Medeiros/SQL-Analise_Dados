-- A proposta do projeto corresponde a criar um dashboard de vendas com os principais indicadores
...
with 
	leads as (
		select
			date_trunc('month', visit_page_date)::date as visit_page_mes,
			count(*) as visit_page_count
		from sales.funnel
		group by visit_page_mes
		order by visit_page_mes
	),
	
	payments as (
		select
			date_trunc('month', fun.paid_date)::date as paid_mes,
...
from leads
left join payments
	on leads.visit_page_mes = paid_mes
